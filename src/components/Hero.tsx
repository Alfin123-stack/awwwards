...
function Hero() {
  ...
  const getVideoSrc = (index) => {
    const safeIndex = ((index - 1 + videosCount) % videosCount) + 1; // selalu 1–4
  ...
  useGSAP(
    () => {
      ...
      // CLICK → EXPAND TRANSITION
      if (hasClicked) {
        gsap.set("#next-video", { visibility: "visible" });

        // expand video animation
        gsap.to("#next-video", {
  ...
  return (
    <div className="relative h-dvh w-screen overflow-x-hidden">
      <div id="video-frame" className="relative h-dvh w-screen overflow-hidden">
        ...
        <div className="absolute inset-0 z-50 flex justify-center items-center">
          <div
            className="mini-video size-64 rounded-lg overflow-hidden cursor-pointer shadow-lg opacity-0 scale-70 hover:opacity-100 hover:scale-100 transition-all duration-500 ease-in-out"
        ...
        {/* Text overlay */}
        <div className="absolute top-10 left-4 p-4 font-circular-web text-white z-50">
          <h1
    ...
    text-sm sm:text-base md:text-lg lg:text-xl">
      ...
      hover:bg-amber-400 hover:text-white shadow-md hover:shadow-amber-200/50">
            <Send
        ...
        {/* Heading besar bawah kanan */}

        <h1
          className="hero-heading absolute bottom-10 right-4 p-4 text-white uppercase font-circular-web font-extrabold
  text-5xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl z-50">
          g<b>a</b>ming
        </h1>
      </div>

      <div className="absolute top-10 left-4 p-4 font-circular-web text-black -z-50">
        <h1
          className="hero-heading uppercase font-extrabold tracking-widest
    text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl">
          redefine
